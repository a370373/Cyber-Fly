...
def _find_data_dir() -> Path:
    """Find a local fly-data directory containing both brain files."""
    roots = []
    ...
    # Then common local storage locations.
    roots.extend([
        Path.home() / "storage",
        Path.home(),
    ])
    ...
    for root in roots:
        ...
        candidates = [root / "fly-data"]

        # Search descendants.
        try:
            candidates.extend(
                p for p in root.rglob("fly-data")
                if p.is_dir()
            )
        ...
        for data in candidates:
            try:
                if all((data / name).is_file() for name in FILES):
                    return data
            except OSError:
                continue

    # No local dataset found; keep a sensible location for first download.
    return project_root / "fly-data"
