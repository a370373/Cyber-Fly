...
def _propagate(W, fired, n):
    """Sum outgoing synaptic weights for neurons that fired.

    CPU backend uses SciPy sparse CSC operations.
    W has rows = postsynaptic neurons and columns = presynaptic neurons.
    """
    if len(fired) == 0:
        return np.zeros(n, dtype=np.float32)

    current = W[:, fired].sum(axis=1)
    return np.asarray(current).ravel().astype(np.float32, copy=False)
...
class FlyBrain:
    """device: "cpu" (SciPy), "cuda" (CuPy, NVIDIA GPU) or "auto"; defaults to
    ...
    def synaptic_input(self, fired):
        ...
        return np.column_stack([_propagate(self._W_cpu, rows[cols == b], self.n)
                                for b in range(B)])
